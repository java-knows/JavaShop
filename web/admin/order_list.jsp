...
<div class="container-fluid">
	...
	<ul role="tablist" class="nav nav-tabs">
		...
		<li <c:if test="${status==4 }">class="active"</c:if> role="presentation"><a href="/admin/order_list?status=4">配送完成</a></li>
		<li <c:if test="${status==5 }">class="active"</c:if> role="presentation"><a href="/admin/order_list?status=5">已确认收货</a></li>
	</ul>
			...
			<tr>
				...
				<td>
					<p>
						...
						<c:if test="${order.status==4 }"><span style="color:deepskyblue;">已到达</span></c:if>
						<c:if test="${order.status==5 }"><span style="color:black;">已收货</span></c:if>
					</p>
				</td>
				<td>
					<p>
						...
						<c:if test="${order.paytype==3 }">银行卡支付</c:if>
					</p>
				</td>
				...
				<td>
					<c:if test="${order.status==2 }">
						<a class="btn btn-primary" href="/admin/order_status?id=${order.id }&status=3">发货</a>
					</c:if>
					<c:if test="${order.status==3 }">
						<a class="btn btn-primary" href="/admin/order_status?id=${order.id }&status=4">送达</a>
					</c:if>
